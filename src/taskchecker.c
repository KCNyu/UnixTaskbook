...
void dataC(char *filename, char *filename2, int *nargs, int tasknum, int testnum)
{
	...
	int usef2 = 0;
	filename2[0] = '\0';
	int num1 = rand() % textdatasize;
	int f1 = open(filename, O_RDWR | O_CREAT, 0777);
	write(f1, textdata[num1], strlen(textdata[num1]));
	lseek(f1, 0, SEEK_SET);
	int f2;
	if (tasknum == 2 || tasknum == 4 || tasknum == 13)
	{
		usef2 = 1;
		for (int i = 0; i < 7; i++)
			filename2[i] = (char)(rand() % 26 + 97);
		filename2[7] = '\0';
		strcat(filename2, testfileext);
		int num2 = rand() % textdatasize;
		while (num2 == num1)
			num2 = rand() % sizeof(textdata);
		f2 = open(filename2, O_RDWR | O_CREAT, 0777);
		write(f2, textdata[num2], strlen(textdata[num2]));
		lseek(f2, 0, SEEK_SET);
	}
	char S[50];
	int slen = rand() % 10 + 20;
	...
	case 2:
		*nargs = 2;
		...
		strcpy(args[2], filename2);
		args[*nargs + 1] = (char *)0;
		while ((n = readline(f1, buf, 200)) > 0)
			write(f, buf, n);
		while ((n = readline(f2, buf, 200)) > 0)
			write(f, buf, n);
		break;
	case 3:
		*nargs = 2;
	...
	case 10:
		*nargs = 1;
		...
		strcpy(args[1], filename);
		args[*nargs + 1] = (char *)0;
		n = readline(f1, buf, 200);
		while ((n2 = readline(f1, buf2, 200)) > 0)
		{
			write(f, buf, n);
			n = n2;
			strcpy(buf, buf2);
